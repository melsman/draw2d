...
struct
 ...
 val green = O.C.stroke_color (O.C.RGB(0,200,0))
...
struct
 ...
 val unitlength_s = "M\\aa{}lestoksforhold: 1:" ^ Int.toString (floor (10.0 / unitlength)) ^ "."
 val red = O.C.stroke_color (O.C.RGB(255,0,0))
 val blue = O.C.stroke_color (O.C.RGB(0,0,255))
 ...
 val vest_facade =
     let val mirr =
             O.all[dotline (0,25) (130,25),
                   ...
                   O.rect (130,100) (850,360),
                   O.line (850,0) (1490,0),
                   O.rect (850,100) (1490,360),
                   O.line (1490,0) (1490,100),
                   O.line (1490,360) (1510,360),
                   O.line (1510,360) (1510,800),
                   O.line (1510,800) (490,800),
                   O.line (1510,390) (880,390),
                   ...
                   O.translate (168,450) kv]
     in
       O.all [O.translate (1800,0) (O.mirror_yaxis mirr),
              O.translate (1100,100) door,
              left //
                   O.all [O.text (0,1000) "{\\large\\textbf{Vestfacade for Matr.\\ nr.\\ 2na, Pcl.\\ 1}}",
                          O.text (0,950) "Peter R\\o{}rdams Vej 24, 2800 Lyngby.",
                          O.text (0,900) ("\\textbf{Version " ^ date ^ "}")
                         ],
              blue //
                   O.all[
                O.rect (610,0) (950,320),
                O.rect (590,320)(1240,400),
                O.line (590,360)(1240,360),
                O.line (950,200)(1240,200),
                O.rect (1060,200) (1080,320),   (* søjle *)
                O.rect (1200,200) (1220,320),   (* søjle *)
                O.line (1240,200) (1380,100),
                O.line (1380,100) (1380,10),
                dotline (950,100) (1240,100),
                dotline (1240,100) (1240,80),   (* trappe start *)
                dotline (1240,80) (1275,80),
                dotline (1275,80) (1275,60),
                dotline (1275,60) (1310,60),
                dotline (1310,60) (1310,40),
                dotline (1310,40) (1345,40),
                dotline (1345,40) (1345,20),
                dotline (1345,20) (1380,20),    (* trappe slut *)
                dotline (640,80)(920,80),
                dotline (640,80)(640,320),
                dotline (920,80)(920,320),
                O.rect (670,260)(890,300)
              ]
             ]
     end
 ...
 val nord_facade =
     O.all [nord_found,
            ...
            measure 50 (120,450) (120,360),
            blue // O.all [O.line (850+wudb,0) (850,0),
                           O.line (850+wudb,0) (850+wudb,100),
                           O.line (850+wudb,100) (850+wudb,350),
                           O.line (850+wudb,100) (850,100),
                           O.line (850+wudb,310) (850,360),
                           O.line (850+wudb,350) (850,400),
                           dotline (850+wudb-30,80) (850+wudb-30,80+240),
                           dotline (850+wudb-30,80) (850,80),
                           O.translate (850+30,100) door,
                           O.all[]
                          ]
           ]
 ...
 val () =
     ...
     in toFile A4 file1 p
     end

 val () =
     let val pict =
             O.all [O.translate (~200,1000) vest_facade,
                    O.translate (~200,0) nord_facade,
                    left // O.all [O.text (~200,~100) "M\\aa{}langivelser er i centimeter. ",
                                   O.text (~200,~150) unitlength_s],
                    O.translate (1300,~100) (W.ruler (SOME("0","1m")))
...
functor GroundFloor(structure P : PICTURE
                    val file : string
                    val paper : P.size
                    val unitlength : real) =
struct
 ...
 val measure = W.measure unitlength
 val red = C.stroke_color (C.RGB(255,0,0))
 val green = C.stroke_color (C.RGB(0,150,0))
 val blue = C.stroke_color (C.RGB(0,0,255))
 val gray = C.stroke_color (C.RGB(200,200,200))
 val orange = O.C.stroke_color (O.C.RGB(15*16+8,8*16,1*16+7)) (*#F88017*)
 val left = C.text_halign C.Left
 ...
 val skorsten_width = 48
 val housei =
     O.all[logogon M.h_sw_p [(N,M.h_width - 2*vaeg),
                             (E,M.u_w_off),
                             (N,M.u_length),
                             (E,M.u_width_w - 2*vaeg),
                             (S,M.u_length_n),
                             (E,M.u_width_e),
                             (S,M.u_length - M.u_length_n - vaeg),
                             (W,60),(S,vaeg),(E,60),
                             (E,M.h_length - M.u_width_w - M.u_width_e),
                             (S,M.h_width - 2*vaeg),
                             (W,488),(S,125),(W,302-2*vaeg),(N,125)],
           logogon M.h_sw_p [(N,14),(E,380),(S,14)] ^^ 372,                        (* bærende væg stue *)
           logogon M.h_sw_p [(N,14),(E,45),(S,14)] ^^ 372 >> (380+skorsten_width), (* skorstensvæg *)
           logogon M.h_sw_p [(N,10),(E,128+10),(S,10+160),(E,288-10),(S,vaeg),
                             (W,25),(S,148),(W,13),(N,148),(W,250),(N,vaeg+160)]
                   ^^ (M.h_width - vaeg + 160) >> M.u_w_off,
           logogon M.h_sw_p [(W,230),(N,365),(E,230),(S,30),(W,200),(S,305),(E,200)]  (* udbygning *)
                   ^^ (M.h_width - vaeg) << (vaeg - M.u_w_off)
          ] >> vaeg ^^ vaeg
 ...
 val trappe = translate trappe_start
                ((*(trin1 ^^ 25) & (trin1 ^^ 50) &*) (trin1 ^^ 65) &
                 (line (112-218,96) (112,96)) &
                 (line (112-218,96) (112-218,200)) &
                 (line (112,66) (112,200)) &
                 (line (0,66) (0,96)) &
                 (line (0,96) (112, 110)) &
                 (line (0,96) (112, 155)) &
 ...
 val symboler =
     pp_list (fn (n,(e,s)) => (e & O.text(30,0) s) vv (n * 40)) "Symboler"
             [(stik, "Stikd\\aa{}se, 20cm over gulv"),
              (kontakt, "Kontakt loftlampe, 110cm over gulv"),
              (stikkontakt, "Stikd\\aa{}se og kontakt, 110cm over gulv"),
              (loftudtag, "Loftlampeudtag"),
 ...
 fun window (len:int) =
     let val vaeg = 30
         val d = 10
     in O.all [(*O.line (d,0) (vaeg-d,len),
               O.line (vaeg-d,0) (d,len),*)
               O.line (d,0) (d,len),
               O.line (vaeg-d,0) (vaeg-d,len),
               O.line (0,0) (vaeg,0),
               O.line (0,len) (vaeg,len)]
     end

 val objs = O.all [blue // O.all[house,housei] << (#1 M.h_sw_p) vv (#2 M.h_sw_p),
                   skorsten ^^ skorsten_y >> skorsten_x,
                   trappe,
                   blue // window 90 ^^ (M.h_width+62) >> M.u_w_off,
                   blue // window 90 ^^ (M.h_width+62+90+90) >> M.u_w_off,
                   blue // window 90 ^^ (M.h_width+M.u_length - 90 - 100) >> M.u_w_off,
                   blue // window 90 @@ (~Math.pi/2.0) ^^ (M.h_width+M.u_length) >> (M.u_w_off + 100),
                   blue // window 90 @@ (~Math.pi/2.0) ^^ (M.h_width+M.u_length) >> (M.u_w_off + 260),
                   blue // window 90 ^^ (M.h_width+M.u_length - 90 - 100) >> (M.u_w_off + M.u_width_w - 30),
                   blue // window 90 @@ (~Math.pi/2.0) ^^ (M.h_width + M.u_length - M.u_length_n)
                        >> (M.u_w_off + M.u_width_w + 50),
                   blue // window 90 ^^ (M.h_width+M.u_length - M.u_length_n - 90 -30)
                        >> (M.u_w_off + M.u_width_w + M.u_width_e - 30),
                   blue // window 90 ^^ (M.h_width+M.u_length - M.u_length_n - 90 -30 -90 - 60)
                        >> (M.u_w_off + M.u_width_w + M.u_width_e - 30)
                  ]
 ...
 val moebler1 =
     green //
     O.all [
       O.rect (0,0) (120,250) ^^ (#2 M.h_sw_p + 200) >> 170,
       O.rect (0,0) (60,300) ^^ (M.h_width + 1) >> (M.u_w_off + M.u_width_w + M.u_width_e - 30 - 60),
       O.rect (0,0) (294,60) ^^ (M.h_width + 1) >> (M.u_w_off + 30 + 128 + 10),
       O.rect (0,0) (90,220) ^^ 250 >> 660,
       O.rect (0,0) (40,120) ^^ (M.h_width - 1 -30 -120) >> (M.u_w_off + 30 + 128 + 10 + 254)
     ]

 val moebler = moebler1

 val pict = O.all [objs >> 250,
                   moebler >> 250,
                   W.ruler (SOME("0","1m")) vv 150,
                   left // all [O.text (750,~100) "M\\aa{}langivelser er i centimeter.",
                                O.text (750,~140) unitlength_s,
                                O.text (0,1400) "{\\large\\textbf{Plantegning stueetage}}",
                                O.text (0,1360) ("\\textbf{Version " ^ date ^ "}")],
                   symboler >> 750 ^^ 1350
                  ]
